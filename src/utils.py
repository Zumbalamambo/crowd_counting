...
def _save_results(input_img, gt_data,density_map,output_dir, fname='results.png'):
...
def save_results(input_img, et, density_map, output_dir, fname='results.png'):
    input_img = input_img[0][0]
    ...
    bottomLeftCornerOfText = (result_img.shape[1]/2 + 10,30)
    fontScale              = 1
    fontColor              = (255,255,255)
    lineType               = 2

    cv2.putText(result_img,'Estimation_count: '+str(et), 
...
def save_density_map(density_map,output_dir, fname='density_map.png'):    
    density_map = 255*density_map/np.max(density_map)
    density_map= density_map[0][0]
